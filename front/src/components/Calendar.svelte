<script lang="ts">
  ...
  let {
    selectedDate = $bindable(startOfDay(today)),
  ...
  // Dados derivados
  // -----------------------
  const currentMonthStart = $derived(startOfMonth(new Date(selectedYear, selectedMonth, selectedDay)));
  ...
  //let inputValue = $state(initialFormatted);
  const calendarYears = differenceInCalendarYears(endDate??addYears(today,11),startDate??new Date(0))
  ...
  function disabledDate(currentDate:Date){
    if(disabled?.before){
      ...
      return isBefore(currentDate,disabled.before)
    }
    if(disabled?.after){
     return isAfter(currentDate,disabled.after)
    }
  ...
  function getPrevMonthTrailingDays() {
    ...
    const prevMonthLastDay = lastDayOfMonth(new Date(selectedYear, selectedMonth - 1));
  ...
  const yearOptions = new Array(calendarYears).fill(0).map((_, i) => startDate?.getFullYear()??new Date(0).getFullYear() + i);
...
<div class={cn("flex  input ",className)}>
    <button  class="cursor-pointer mr-3 rounded-sm">
      <X
        onclick={() => {
          if(disabled?.before==selectedDate){
             inputValue = format(today,'dd/MM/yyyy')
   ...
   <input
     id="input"
      ...
      oninput={() => {
        inputValue = normalizeDateInput(inputValue, today,yearOptions[yearOptions.length-1],yearOptions[0]);
      }}
    />
